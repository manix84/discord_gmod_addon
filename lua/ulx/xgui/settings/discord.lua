...
net.Receive("discordAvailableLanguages", function()
  ...
  for i, languageName in ipairs(availableLanguagesTable) do
    ...
    discord_settings_config_Language_combobox:AddChoice(languageTitle, languageName, isSelected);
...
------------------------Settings Module-----------------------
local discord_bot_connection_panel = xlib.makelistlayout{
  w = 415,
  h = 318,
  parent = discord.panel
};
...
-------------------------Players Module-----------------------
local discord_playerConnections_panel = xlib.makelistlayout{
  w = 415,
  h = 318,
  parent = discord.panel
};
...
discord_playerConnections_table_List.listview.OnRowSelected = function(self, LineID, Line)
  ...
  discord_playerConnections_DiscordID_textBox:SetText(steamIDToDiscordIDConnectionTable[discord_playerConnections_table_List__Selected_SteamID] or '');
end;
...
net.Receive("discordPlayerTable", function()
  ...
  for index, target_ply in pairs(player.GetAll()) do
    if not target_ply:IsBot() then
      discord_playerConnections_table_List.listview:AddLine(target_ply:GetName(), target_ply:GetUserGroup(), steamIDToDiscordIDConnectionTable[target_ply:SteamID()], target_ply:SteamID()); -- Hidden SteamID column
    end
  end
end);
